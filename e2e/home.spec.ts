...
test.describe("Home page", () => {
  test("loads and shows the blog title", async ({ page }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: /Jaiquez Book Blog/i }),
    ).toBeVisible();
  });
  ...
  test("navigates to the add entry form when the Fab is clicked", async ({
    page,
  }) => {
    await page.goto("/");
...
test.describe("Find Entry page", () => {
  ...
  test("shows the Find Book heading", async ({ page }) => {
    await expect(
      page.getByRole("heading", { name: /Find Book/i }),
    ).toBeVisible();
  });
